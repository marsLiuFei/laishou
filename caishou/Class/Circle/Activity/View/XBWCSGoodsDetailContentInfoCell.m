...
-(void)setHtmlString:(NSString *)htmlString{
    ...
    [self.wkWebView loadHTMLString:[self reSizeImageWithHTML:_htmlString] baseURL:nil];
}
...
- (void)webView:(WKWebView *)webView didFinishNavigation:(WKNavigation *)navigation{
    ...
    [webView evaluateJavaScript:@"document.body.scrollHeight" completionHandler:^(id data, NSError * _Nullable error) {
        CGFloat height = [data floatValue];//document.body.offsetHeight
...
-(nullable NSString *)reSizeImageWithHTML:(NSString *)html {
    return [NSString stringWithFormat:@"<meta name='viewport' content='width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=0'><meta name='apple-mobile-web-app-capable' content='yes'><meta name='apple-mobile-web-app-status-bar-style' content='black'><meta name='format-detection' content='telephone=no'><style type='text/css'>img{width:%fpx}</style>%@", SCREEN_WIDTH-SIZE(15), html];
}

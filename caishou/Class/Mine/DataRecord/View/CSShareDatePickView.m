...
@interface CSShareDatePickView()<UIPickerViewDelegate,UIPickerViewDataSource>
{
    UIView *_contentView;
}
...
@implementation CSShareDatePickView
-(NSArray *)sourceDataArray{
    if (!_sourceDataArray) {
        _sourceDataArray = [NSArray new];
        NSMutableArray *arrM = [NSMutableArray new];
        for (int i=0; i<365; i++) {
            [arrM addObject:[NSString stringWithFormat:@"%d天",i+1]];
        }
        _sourceDataArray = arrM;
    }
...
//    self.pickerDateStr = [NSString stringWithFormat:@"%@", [self.sourceDataArray objectAtIndex:self.selectedIndex]];
    NSString *result = [NSString stringWithFormat:@"%ld",self.selectedIndex+1];
    if (self.pickerBlock) {
        self.pickerBlock(result);
    }

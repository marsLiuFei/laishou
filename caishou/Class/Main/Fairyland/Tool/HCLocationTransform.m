//
//  HCLocationTransform.m
//  BaiXinXueChe
//
//  Created by 微微笑了 on 2017/6/23.
...
@implementation HCLocationTransform
...
- (id)transformFromGPSToGD {
    CLLocationCoordinate2D coor = [HCLocationTransform transformFromWGSToGCJ:CLLocationCoordinate2DMake(self.latitude, self.longitude)];
    return [[HCLocationTransform alloc] initWithLatitude:coor.latitude andLongitude:coor.longitude];
}

- (id)transformFromGDToBD {
    CLLocationCoordinate2D coor = [HCLocationTransform transformFromGCJToBaidu:CLLocationCoordinate2DMake(self.latitude, self.longitude)];
    return [[HCLocationTransform alloc] initWithLatitude:coor.latitude andLongitude:coor.longitude];
}

- (id)transformFromBDToGD {
    
    MYLog(@"self.latitude = %lf", self.latitude);
    MYLog(@"self.longitude = %lf", self.longitude);
    
    CLLocationCoordinate2D coor = [HCLocationTransform transformFromBaiduToGCJ:CLLocationCoordinate2DMake(self.latitude, self.longitude)];
    return [[HCLocationTransform alloc] initWithLatitude:coor.latitude andLongitude:coor.longitude];
}

- (id)transformFromGDToGPS {
    CLLocationCoordinate2D coor = [HCLocationTransform transformFromGCJToWGS:CLLocationCoordinate2DMake(self.latitude, self.longitude)];
    return [[HCLocationTransform alloc] initWithLatitude:coor.latitude andLongitude:coor.longitude];
}
...
- (id)transformFromBDToGPS {
    ...
    CLLocationCoordinate2D start_coor = [HCLocationTransform transformFromBaiduToGCJ:CLLocationCoordinate2DMake(self.latitude, self.longitude)];
    CLLocationCoordinate2D end_coor = [HCLocationTransform transformFromGCJToWGS:CLLocationCoordinate2DMake(start_coor.latitude, start_coor.longitude)];
    return [[HCLocationTransform alloc] initWithLatitude:end_coor.latitude andLongitude:end_coor.longitude];
}

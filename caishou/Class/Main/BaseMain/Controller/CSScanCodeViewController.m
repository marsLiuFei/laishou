...
@interface CSScanCodeViewController ()
{
...
@implementation CSScanCodeViewController
...
- (void)setupQRCodeScan {
    ...
    [obtain setBlockWithQRCodeObtainScanResult:^(SGQRCodeObtain *obtain, NSString *result) {
        if (result) {
            ...
            if ([[dic allKeys] containsObject:@"clubId"]) {
                //俱乐部
                [weakSelf bindClubWithCode:dic[@"clubId"]];
            }
            else if ([[dic allKeys] containsObject:@"counselorId"]){
                //顾问
                [weakSelf bindCounselorIdWithCode:dic[@"counselorId"]];
            }
        }
    }];
...
//绑定俱乐部
- (void )bindClubWithCode:(NSString *)code{
    ws(bself);
    [CSApiManager BindingClubWithID:code CallBack:^(id  _Nonnull data) {
        [Dialog toastCenter:BackMessage];
        if (ResultIsCorrect) {
            if (bself.needFreshBlock) {
                bself.needFreshBlock();
            }
...
//绑定顾问
- (void )bindCounselorIdWithCode:(NSString *)code{
    ws(bself);
    [CSApiManager BindingCounselorWithID:code CallBack:^(id  _Nonnull data) {
        [Dialog toastCenter:BackMessage];
        if (ResultIsCorrect) {
            if (bself.needFreshBlock) {
                bself.needFreshBlock();
...
//        [[Dialog Instance] showCenterProgressWithLabel:@"正在处理..."];
//        if (result == nil) {
//            NSLog(@"暂未识别出二维码");
//            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
//                [[Dialog Instance] hideProgress];
//                [Dialog toastCenter:@"未发现二维码/条形码"];
//            });
//        } else {
//            //扫码结果
//            MYLog(@"%@",result);

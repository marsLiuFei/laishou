...
- (void)showTitle:(NSString *)title xOffset:(CGFloat)x yOffset:(CGFloat)y
{
    MBProgressHUD *hud = [self getMBProgressHUD];
    if (hud == nil) {
        hud = [self createHUDIndicatorViewAtCenter:title xOffset:x yOffset:y imageName:nil contentType:TextContentType];
    }else{
        [self setTextView:hud title:title];
    }
    [hud showAnimated:YES];
...
- (void)showLoadMessageAtCenter{
    [self showLoadMessageAtCenter:@"加载中..." yOffset:-navgationBarHeight/2.0];
}
- (void)showLoadMessageAtCenter:(NSString *)title{
    //键盘会遮盖加载动画，让键盘失去响应
    [self endEditing:YES];
    [self showLoadMessageAtCenter:title yOffset:-navgationBarHeight/2.0];
}

 ...
 * datasource 数据源
 */
- (instancetype)initWithFrame:(CGRect)frame dataSource:(NSArray<NSString *>*)dataSource  countFoyY:(NSInteger)countY xSource:(NSArray<NSString *>*)xSource{
...
- (instancetype)initWithFrame:(CGRect)frame countFoyY:(NSInteger)countY {
    ...
    return [self initWithFrame:frame dataSource:nil countFoyY:countY xSource:nil];
}
...
- (void)resetUIAndData {
    ...
    NSInteger page = _dataSource.count / 4 + 1;
...
-(void)addDataPoints {
    ...
    for (int i = 0; i<arr.count; i++) {
        ...
        UIButton *btn = [[UIButton alloc]initWithFrame:CGRectMake((_xMargin)*i-_xMargin*0.5, height *(1 - percent) - btnW/2 , btnW, btnW)];
...
//            btn.hidden = NO;
        ...
        [detailLabel setFrame:CGRectMake((_xMargin)*i-_xMargin*0.5-textSize.width/2+btnW/2, height *(1 - percent)-30 , textSize.width, textSize.height)];
...
-(void)addBottomViews {
    ...
    for (int i = 0;i< _dataArrOfX.count ;i++ ) {
        ...
        UILabel *leftLabel = [[UILabel alloc]initWithFrame:CGRectMake(5+_xMargin/2+i*_xMargin-_xMargin*0.5, (_countY - 1)*_yMargin, _xMargin, 40)];
